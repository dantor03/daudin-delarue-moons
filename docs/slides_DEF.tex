...
\begin{center}
  \includegraphics[width=0.92\linewidth]{../figuras/E_parameter_analysis.png}
\end{center}
{\footnotesize\textit{Fila 0: histogramas marginales de $a_1^m$, $a_2^m$, $a_0^m$. Fila 1: scatter 2D de $a_1^m$ coloreado por importancia $\|a_0^m\|_2$. Fila 2: scatter $c_m(0)$ vs $c_m(T)$, importancias ordenadas, correlación entrada--salida.}}
...
\end{center}
{\footnotesize\textit{Filas: F1 (datos variables), F2 (init variable). Columnas: pérdida, scatter 2D de $a_1^m$, histograma de ángulos $\theta$. Fila 2: $\bar{R}$ por run, escala de $\|a_1^m\|$, importancias ordenadas.}}
...
\begin{block}{Importancias: estructura universal}
  Codo rápido en $\|a_0^m\|_2$ tanto en circles como en moons: propiedad de la \textbf{arquitectura}, no del dataset.
\end{block}
\end{columns}
